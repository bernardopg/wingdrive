...
WingDrive extensions are designed to be powerful, but also secure and well-behaved citizens of the user's digital ecosystem. This is achieved through a multi-layered security model and deep integration with WingDrive's native sync capabilities.

### Migration Security

When an extension needs to change its database schema, it does so through a migration system that is carefully controlled by the WingDrive core.
...
Extension models are first-class participants in WingDrive's leaderless, hybrid sync system. This means that data created by an extension on one device will automatically sync to all other devices.

### Sync Strategies

WingDrive uses two primary sync strategies, and extensions can choose the appropriate one for each of their models:
...
- **Shared Infrastructure**: Extension models use the same sync infrastructure (database tables and network protocol) as core WingDrive models. The `model_type` field in the sync log distinguishes between core models and extension models.
