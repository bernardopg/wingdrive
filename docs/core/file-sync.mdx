...
File Sync orchestrates content synchronization between locations by leveraging WingDrive's Virtual Distributed File System (VDFS) index. Unlike traditional sync tools that scan filesystems, File Sync operates entirely through index queries. This makes sync state resolution a simple database operation and ensures perfect consistency with your indexing rules.

## Architecture

File Sync builds on WingDrive's core infrastructure. Each sync operation runs as a Job, providing state persistence and progress tracking. The system derives all sync decisions from the VDFS index, eliminating complex state management and ensuring files ignored during indexing remain excluded from sync.
...
**Index as Truth**  
If a file isn't indexed, it doesn't exist for sync purposes. This ensures consistency between what you see in WingDrive and what gets synchronized.
...
The system uses last-write-wins for conflicts. When the same file changes in multiple places, the most recent modification takes precedence. Previous versions remain in your WingDrive history.
...
Use this mode when finishing projects. Your files move to cheaper storage while maintaining full WingDrive search and preview capabilities.

### Selective Mode
Intelligently manages limited local storage. Frequently accessed files stay local while older content moves to secondary storage. Files remain visible in WingDrive and download on-demand when needed.

The algorithm considers access patterns, available space, and manual pins. You control which files always stay local by pinning them in the WingDrive interface.
...
<Info>
Files excluded from indexing (like `node_modules`) are automatically excluded from sync. This matches user expectations - if you told WingDrive to ignore it, it won't sync either.
</Info>
...
File transfers use WingDrive's existing P2P infrastructure over QUIC. The protocol supports:
...
Create sync relationships through the WingDrive interface by right-clicking any directory and selecting "Sync to...". The system creates a SyncConduit record linking your source and destination.
...
**Consistency**  
Files ignored by indexing rules automatically stay out of sync. No divergence between what WingDrive shows and what gets synchronized.
