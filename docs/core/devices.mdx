...
Devices are machines running WingDrive. Each device has a unique identity, can pair with others, and participates in library synchronization.
...
- macOS: `~/Library/Application Support/com.wingdrive/device.json`
- Linux: `~/.config/wingdrive/device.json`
- Windows: `%APPDATA%/WingDrive/device.json`
...
Each device has a unique slug used in WingDrive's unified addressing scheme. The slug is a URL-safe identifier generated from the device name:
...
<Info>
	WingDrive uses a leaderless sync model. All devices are peers with no central
	authority.
</Info>
...
sqlite3 ~/WingDrive/Libraries/My\ Library.sdlibrary/database.db \
...
sqlite3 ~/WingDrive/Libraries/My\ Library.sdlibrary/sync.db \
