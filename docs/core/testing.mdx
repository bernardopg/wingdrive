...
Testing in WingDrive Core ensures reliability across single-device operations and multi-device networking scenarios. This guide covers the available frameworks, patterns, and best practices.

## Testing Infrastructure

WingDrive Core provides two primary testing approaches:
...
WingDrive provides two approaches for testing multi-device scenarios:
...
let test_root = PathBuf::from(home).join(".wingdrive_test_my_test");
...
**Naming convention**: `wingdrive-test-{test_name}`

```rust
// ✅ CORRECT: Platform-aware temp directory
let test_data = TestDataDir::new("file_operations")?;
// Creates: /tmp/wingdrive-test-file_operations/ (Unix)
//      or: %TEMP%\wingdrive-test-file_operations\ (Windows)

// ❌ INCORRECT: Hardcoded paths outside temp
let test_dir = PathBuf::from("~/Library/Application Support/wingdrive/tests");
...
```
~/Library/Application Support/wingdrive/test_snapshots/  (macOS)
~/.local/share/wingdrive/test_snapshots/                 (Linux)
%APPDATA%\wingdrive\test_snapshots\                      (Windows)
```
...
WingDrive maintains a curated suite of core integration tests that run in CI and during local development. These tests are defined in a single source of truth using the `xtask` pattern.
...
Core integration tests use the WingDrive source code itself as test data instead of user directories. This ensures:
...
```rust
// Tests index the WingDrive project root
...
let location = harness
    .add_and_index_location(test_path.to_str().unwrap(), "wingdrive")
    .await?;
```
...
4. **Use deterministic test data**: Index WingDrive source code instead of user directories

### Test Data

1. **All test data in temp directory**: Use `TestDataDir` or `TempDir` (see Test Data & Snapshot Conventions)
2. **Prefer project source code**: Use `env!("CARGO_MANIFEST_DIR")` to locate the WingDrive repo for test indexing
...
- [ ] Test uses deterministic data (WingDrive source code)
...
WingDrive provides a bridge infrastructure for running TypeScript tests against a real Rust daemon. This enables true end-to-end testing across the Rust backend and TypeScript frontend, verifying that cache updates, WebSocket events, and React hooks work correctly with real data.
...
4. **TypeScript test** reads config, connects to daemon via `WingDriveClient.fromTcpSocket()`
...
- `tests/sync_realtime_test.rs` - Real-time sync testing with deterministic transport using WingDrive source code
