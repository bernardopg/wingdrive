...
**Non-Destructive**: WingDrive never modifies your original files during indexing.
...
**Managed Sidecars** - WingDrive-generated derivatives stored in `.sdlibrary/sidecars/`:
- Thumbnails (`.webp`)
- Video proxies (`.mp4`)
- OCR text (`.json`)
...
The VSS uses two tables in the WingDrive database:
...
The `SidecarManager` provides a set of APIs for interacting with the VSS. These are primarily for internal use by other WingDrive services, but they can also be used for development and debugging.
