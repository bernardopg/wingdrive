...
ZFS is the most-developed filesystem integration because TrueNAS Scale is a common WingDrive server target.
...
WingDrive tracks far more volumes than it shows. Hidden volumes still get stable fingerprints so locations on them survive remounts, but they don't clutter the default UI and aren't eligible for auto-tracking.
