...
WingDrive detects and tracks storage volumes across all platforms, including local drives and cloud storage. The volume system enables intelligent file operations by understanding where data lives and how to move it efficiently.
...
When you connect a drive or add cloud storage, WingDrive detects it immediately. If you choose to track that volume, it remembers your preferences and recognizes it when reconnected.

## Storage Backends

WingDrive supports two types of storage backends:
...
WingDrive uses service-native URIs for cloud volumes that match industry tools:
...
### Filesystems
WingDrive recognizes major filesystems including APFS, NTFS, Ext4, Btrfs, ZFS, FAT32, and ExFAT.
...
WingDrive uses optimal copy strategies based on the storage backend:
...
WingDrive uses consistent content hashing across all storage backends:
