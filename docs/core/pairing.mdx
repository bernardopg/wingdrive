...
Device pairing establishes trust between WingDrive instances using cryptographic signatures and user-friendly codes. Once paired, devices can communicate securely and share data directly.
...
WingDrive uses BIP39 mnemonic codes for pairing, which come in two formats:
...
For pairing across networks, WingDrive uses pkarr to publish and resolve node addresses via DNS:
...
When using QR codes (with node_id), WingDrive races both discovery methods:
...
WingDrive uses pkarr for decentralized node address resolution:
...
- Firewalls allow WingDrive traffic
