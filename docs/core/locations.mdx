...
Locations are directories that WingDrive tracks and monitors. When you add a location, WingDrive indexes its contents and watches for changes in real-time.

## What is a Location?

A location is any folder on your device that you want WingDrive to index. Once added:
...
When you add a location, WingDrive creates a **root entry** in the entries table for the location directory itself. This root entry:
...
Choose how deeply WingDrive analyzes your files:
...
```bash
# Add a location
wingdrive-cli location add ~/Documents --name "Documents"

# Add with deep indexing
wingdrive-cli location add ~/Photos --name "Photos" --mode deep

# List all locations
wingdrive-cli location list
```
...
```bash
wingdrive-cli location pause <location-id>
```
...
```bash
# Disable watching
wingdrive-cli location update <location-id> --watch false

# Change index mode
wingdrive-cli location update <location-id> --mode shallow
```
...
```bash
wingdrive-cli location remove <location-id>
```

<Warning>
  Removing a location doesn't delete files. It only stops WingDrive from
  tracking them.
</Warning>
...
wingdrive-cli location list --verbose

# Disable problematic location
wingdrive-cli location update <id> --watch false
```
...
```bash
# Set debug mode
export WINGDRIVE_WATCHER_DEBUG=1

# Run with verbose logging
wingdrive-cli --log-level debug
```
