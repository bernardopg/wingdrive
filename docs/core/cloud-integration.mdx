...
WingDrive treats cloud storage as native volumes, enabling seamless file management across local and cloud storage. The system leverages OpenDAL to support over 40 cloud services while maintaining consistent performance and behavior.

## How Cloud Volumes Work

Cloud volumes appear and function identically to local volumes in WingDrive. When you connect a cloud service, it becomes a volume with the same capabilities as your local drives. Files are indexed, searchable, and manageable without downloading entire contents.

The key innovation is content identification consistency. Whether a file lives on your SSD or in S3, WingDrive generates the same content hash, enabling accurate deduplication and tracking across all storage locations.
...
dropbox://Apps/WingDrive/backup.zip
azblob://container/data/export.csv
gcs://bucket/logs/app.log
```

These URIs match the format used by AWS CLI, gsutil, and other cloud tools. Copy a path from WingDrive and paste it directly into cloud provider CLI commands.

Internally, WingDrive uses the `SdPath` enum with identity-based addressing:
...
WingDrive supports all major cloud providers through OpenDAL:
...
Each service maintains its native features while presenting a unified interface through WingDrive.

## Content Identification

WingDrive uses its sampling algorithm for all files, regardless of storage location. For files over 100KB, only 58KB of data transfers to generate the content hash:
...
<Info>
	WingDrive never stores credentials in plain text or transmits them outside
...
When files change outside WingDrive:
...
**File Watcher**: Cloud volumes do not support real-time file monitoring. Changes made outside WingDrive require manual refresh or reindexing to appear.
...
- [Indexing](/docs/core/indexing) - How WingDrive indexes files
