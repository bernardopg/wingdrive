...
WingDrive's indexing system solves a specific challenge: How do you build a distributed database that feels as fast as a local file explorer?

The answer is a **Hybrid Indexing Engine** that layers an ultra-fast, in-memory ephemeral index over a robust, SQLite-backed persistent index. These two systems operate in tandem, allowing WingDrive to instantly browse unmanaged locations (like a file manager) while seamlessly upgrading those paths to managed libraries (like a DAM) without UI flicker or state loss.

## The Hybrid Philosophy

Most file management software forces a choice: fast, dumb directory listing (Explorer/Finder) or slow, heavy database ingestion (Lightroom/Photos). WingDrive does both simultaneously by decoupling **Discovery** from **Persistence**.

### The Ephemeral Layer ("File Manager" Mode)

When you open a location that hasn't been added to your library—an external drive, network share, or local directory—WingDrive runs only Phase 1 (Discovery) of the indexing pipeline:
...
For files you want to track across devices, WingDrive persists data to a synchronized SQLite database using the full multi-phase pipeline with deep content analysis, deduplication, and closure-table hierarchy management.
...
<Info>
This architecture allows WingDrive to act as your daily driver file explorer. You get instant access to files immediately, with the option to progressively "deepen" the index for files that matter.
</Info>
...
The system integrates deeply with WingDrive's job infrastructure, which provides automatic state persistence through MessagePack serialization. When you pause an indexing operation, the entire job state is saved to a dedicated jobs database, allowing seamless resumption even after application restarts.
...
Enables WingDrive's deduplication capabilities through Content Addressable Storage (CAS):
...
WingDrive supports both persistent and ephemeral indexing modes:
...
- **MissingFromIndex**: Files created outside WingDrive
...
The indexing system leverages WingDrive's job infrastructure for reliability and monitoring.
...
wingdrive-cli index quick-scan ~/Documents

# Browse external drive
wingdrive-cli index browse /media/usb --ephemeral

# Full location with progress monitoring
wingdrive-cli index location ~/Pictures --mode deep
wingdrive-cli job monitor  # Watch progress
```
...
```bash
RUST_LOG=sd_core::ops::indexing=debug wingdrive-cli start
```

Inspect job state:

```bash
wingdrive-cli job info <job-id> --detailed
```
