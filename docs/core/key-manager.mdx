...
The KeyManager is WingDrive's unified cryptographic secret storage system. It provides encrypted storage for all sensitive data including device keys, library encryption keys, paired device session keys, and cloud credentials.
...
1. **Primary**: OS Keychain
   - macOS: Keychain Access (`WingDrive` service)
...
<Info>
  Prior to the unified KeyManager, WingDrive stored secrets in multiple locations:
...
```bash
# Backup first
cp ~/.wingdrive/secrets.redb ~/.wingdrive/secrets.redb.backup

# Remove corrupted database (will lose paired devices!)
rm ~/.wingdrive/secrets.redb

# Restart WingDrive (new database will be created)
```
...
```bash
# Check permissions
ls -la ~/.wingdrive/secrets.redb
ls -la ~/.wingdrive/device_key  # If using fallback

# Fix if needed
chmod 600 ~/.wingdrive/secrets.redb
chmod 600 ~/.wingdrive/device_key
```
...
security find-generic-password -s "WingDrive" -a "device_key" -w

# Linux: Backup entire secrets database
cp ~/.wingdrive/secrets.redb ~/backup/

# Windows: Export from Credential Manager
cmdkey /list | findstr WingDrive
```
