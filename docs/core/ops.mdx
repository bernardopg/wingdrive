...
```swift
let library = try await wingdrive.libraries.create(
...
```typescript
const library = await wingdrive.libraries.create({
	name: "My Library",
});
```
...
fn main() {
    ...
    fs::write("WingDriveTypes.swift", swift_types)?;
    fs::write("WingDriveAPI.swift", api_methods)?;
}
```
