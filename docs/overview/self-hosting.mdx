---
title: Self-Hosting WingDrive
description: Deploy WingDrive server for remote access and headless operation
sidebarTitle: Self-Hosting
---

Self-hosting WingDrive means running the server component on hardware you control. The server provides HTTP access to your libraries, enabling remote connections from desktop and mobile apps. Unlike the desktop app which requires a GUI, the server runs headless on Linux systems, NAS devices, or cloud instances.

The server embeds the full WingDrive daemon with media processing capabilities. It exposes RPC endpoints over HTTP with optional basic authentication. Your data stays on your infrastructure while remaining accessible from any device with network access.

## Installation Methods

### Static Binary

Download the latest x86_64 Linux release binary. This method requires no container runtime.

```bash
# Download the binary and release checksums
wget https://github.com/bernardopg/wingdrive/releases/latest/download/wingdrive-server-linux-x86_64
wget https://github.com/bernardopg/wingdrive/releases/latest/download/SHA256SUMS
...
For ARM systems like Raspberry Pi, use the multi-architecture Docker image below.
...
Create a service file at `/etc/systemd/system/wingdrive.service`:
...
```bash
sudo useradd -r -s /bin/false wingdrive
sudo mkdir -p /var/lib/wingdrive
sudo chown wingdrive:wingdrive /var/lib/wingdrive
sudo systemctl enable --now wingdrive
```

Check status with `sudo systemctl status wingdrive`.
...
server {
    ...
    server_name wingdrive.example.com;

    ssl_certificate /etc/letsencrypt/live/wingdrive.example.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/wingdrive.example.com/privkey.pem;
...
Network-attached storage devices are ideal for WingDrive server. The server indexes existing media while providing remote access.
...
After deployment, create locations in WingDrive pointing to these mount points. The server indexes files without moving them.
...
Add a custom container template with the image `ghcr.io/bernardopg/wingdrive/server:latest`. Map your shares as additional paths in the container configuration.
...
The URL format is `http://your-server:8080` or `https://wingdrive.example.com` if using a reverse proxy. Clients authenticate using credentials set in `SD_AUTH`.
...
```bash
sudo systemctl stop wingdrive
sudo mv sd-server-new /usr/local/bin/sd-server
sudo systemctl start wingdrive
```
...
Report issues on [GitHub](https://github.com/bernardopg/wingdrive/issues) with server logs and deployment details. Join [Discord](https://discord.gg/gTaF2Z44f5) for community support and deployment advice.
