...
WingDrive provides type-safe React hooks for data fetching, mutations, and event subscriptions. All types are auto-generated from Rust definitions.
...
Subscribe to specific WingDrive events.
...
Subscribe to all WingDrive events (useful for debugging).
...
Access the WingDrive client instance directly.

```tsx
import { useWingDriveClient } from '@sd/interface';
...
WingDrive's React hooks provide:
