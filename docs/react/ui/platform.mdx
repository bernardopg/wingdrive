...
The platform abstraction layer enables the WingDrive interface to run on multiple platforms (Web, Tauri, React Native) while accessing platform-specific features.
...
function HelpButton() {
  ...
  return (
    <button onClick={() => platform.openLink('https://github.com/bernardopg/wingdrive/tree/main/docs')}>
      Open Documentation
    </button>
  );
}
