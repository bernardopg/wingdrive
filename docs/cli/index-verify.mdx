...
The `index verify` command performs a comprehensive integrity check of the WingDrive index for any given path. It compares the actual filesystem state with what's stored in the database and reports any discrepancies.
...
Queries the WingDrive database for all indexed entries under the given path:
...
If you've manually modified files outside WingDrive:
